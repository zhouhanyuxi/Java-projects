	      	...
	      	<div class="layui-inline">
			      <div class="layui-inline">
			          ...
			          <div class="layui-input-block">
			            <input type="text" name="cusMonIORecCode" placeholder="请输入" autocomplete="off" class="layui-input">
			          </div>
			        </div>
			        ...
			        <div class="layui-inline" >
			          <label class="layui-form-label">产生原因</label>
			          <div class="layui-input-block">
			          	<select name="reason" lay-verify="required" lay-filter="peroid">
			          		<option value="PF" selected>批发</option>
							<option value="TH">退货</option>
							<option value="QX">取消</option>
	 ...
	 /*    opComplain:function(d){
		...
		renderListTable:function(){
			...
			table.render({
			    ...
			    cols:[[ //表头
	                {field: 'cusMonIORecCode', title: '记录编号', sort: true },
	                {field: 'wholesaleOrdCode', title: '订单编号', sort: true },
	                {field: 'username', title: '用户名', sort: true },
	                {field: 'reason', title: '记录产生原因', sort: true 
	                	,templet: function(row){
	                    	if(row.reason == 'PF'){
	                    		return '<span style="color: #c00;">批发</span>'
	                    	}else if(row.reason == 'TH'){
	                    		return '<span style="color: #c00;">退货</span>'
	                    	}else if(row.reason == 'QX'){
	                    		return '<span style="color: #c00;">取消</span>'
	                    	}/* else if(row.reason == 'QT'){
