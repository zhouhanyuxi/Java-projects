	 ...
	 /*    opComplain:function(d){
		...
		renderListTable:function(){
			...
			table.render({
			    ...
			    cols:[[ //表头
	                {field: 'OrdMonRecCode', title: '记录编号', sort: true },
	                {field: 'OrdMonRecCode', title: '记录类型', sort: true 
	                	,templet: function(row){
	                    	if(row.OrdMonRecCode.substring(0,1) == 'C'){
	                    		return '<span style="color: #c00;">采购</span>'
	                    	}else if(row.OrdMonRecCode.substring(0,1) == 'P'){
